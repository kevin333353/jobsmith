...
macOS 目前提供 **unsigned** `.app.zip` 建置流程，尚未做 Apple Developer ID 簽章與 notarization。第一次開啟可能被 Gatekeeper 擋下，請右鍵 `Jobsmith.app` →「打開」，或到系統設定允許。

開發者可到 GitHub **Actions → Build unsigned macOS app** 手動執行 workflow，會產出：

- `Jobsmith-macOS-arm64-unsigned.zip`：Apple Silicon（M1/M2/M3/M4）
- `Jobsmith-macOS-x64-unsigned.zip`：Intel Mac

workflow 也提供 `publish_release=true` 選項，可把上述 zip 與 sha256 覆蓋到指定 release tag。macOS 版資料與 `.env` 會存在 `~/Library/Application Support/Jobsmith`。
...
| 桌面     | pywebview（原生視窗）· PyInstaller（單檔 `.exe` / unsigned `.app`）          |
